...
int32_t pixelcolor(t_cub *cub, double x, double y) {
  ...
  color = 0;
  if (factor < 1024 * 1024) {
    color = ft_pixel(cub->txt->pixels[i], cub->txt->pixels[i + 1],
...
void draw_view(t_cub *cub) {
  ...
  while (i < WIDTH) {
    distance = sqrt((pow((cub->point[i].x_end / 30) - (cub->par.x / 30), 2)) +
    ...
    // printf ("%d\n", p);
      ...
      if (x > 0 && x < WIDTH && u < cub->txt->width){
        mlx_put_pixel(cub->mlx.img_ptr, i, x, cub->color_texture[p][u]);
      }
      if (p < cub->txt->height)
        break;
      p++;
...
void load_textures(t_cub *cub) 
{
  ...
  if (!cub->img)
    return;
  cub->color_texture = malloc(sizeof(int32_t *) * cub->txt->height);
  if (!cub->color_texture)
    return ;
  while(i < cub->txt->height)
  {
      cub->color_texture[i] = malloc(sizeof(int32_t) * cub->txt->width);
